r"""
学术组图工具 V18 - 启动器 (Smart Grid · Proportional Fill Edition)
...
使用方式:
    python run_v18.py [theme]                 交互式启动
    python run_v18.py path\to\project.figbox  直接打开项目（双击场景）
    python run_v18.py [theme] path\to\x.figbox  指定主题并打开项目
...
def main():
    ...
    import logging
    logger = logging.getLogger("run_v18")
    logger.info("FigBox V18 starting; theme=%s, project=%s, log=%s",
                theme_key, project_path, log_path)

    app = QApplication(sys.argv)
    app.setApplicationName("学术组图工具 V18 FigBox")
    ...
    window.setWindowTitle(f"学术组图工具 V18.0 FigBox - {theme_name}")
    window.show()
