...
def export_combined_pdf(layouts: List[LayoutItem],
    ...
    for layout in layouts:
        ...
        # Add label (A, B, C, etc.)
        add_label(output_page, layout, x_pt, y_pt, dpi, label_bold, label_offset)
...
def add_label(page, layout: LayoutItem, x_pt: float, y_pt: float, dpi: int, bold: bool = True, label_offset: float = 0.25):
    """
...
def export_combined_image(layouts: List[LayoutItem],
    ...
    for idx, layout in enumerate(layouts):
        try:
            ...
            # V18 性能优化：直接按"目标像素尺寸"渲染，而不是先按 max(dpi/72,…) 过度渲染
            ...
            # Add label
            add_label_to_image(draw, layout, x_px, y_px, mm_to_px, label_bold, label_offset)
...
def add_label_to_image(draw, layout: LayoutItem, x_px: int, y_px: int, mm_to_px: float, bold: bool = True, label_offset: float = 0.25):
    """
    ...
    """
    label_text = layout.label
    font_size = int(12 * mm_to_px / (300 / 72))  # Scale font size based on DPI
