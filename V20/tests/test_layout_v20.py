...
class LayoutEngineV20Tests(unittest.TestCase):
...
class SettingsManagerV20Tests(unittest.TestCase):
    def test_default_settings_include_v20_defaults(self):
    ...
    def test_save_and_load_settings_merges_with_defaults(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "settings_v20.json")
...
class PDFInfoV20Tests(unittest.TestCase):
...
class ExportCanvasBoundsTests(unittest.TestCase):
    ...
    def test_export_canvas_includes_top_label_bounds(self):
        layouts = [FakeItem(40, 30, "A")]
        layouts[0].x = 5
        layouts[0].y = 5
        fontsize = 12
        offset = 0.25
        ...
        shifted, width, height = calculate_export_canvas(
            layouts, 297, 210, padding=2, auto_crop=True,
            label_visible=True, label_fontsize=fontsize,
            label_offset=offset, label_bold=True)

        font = fitz.Font(fontname="hebo")
        label_top = (
            shifted[0].y - offset
            - font.ascender * fontsize * 25.4 / 72.0
        )
        self.assertGreaterEqual(label_top, 2 - 1e-6)
        self.assertGreater(shifted[0].y, 6)
        self.assertGreater(height, 34)
...
class StartupBehaviorTests(unittest.TestCase):
    def test_startup_does_not_prompt_to_recover_autosaves(self):
        run_script = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "run_v20.py")
        with open(run_script, "r", encoding="utf-8") as f:
            source = f.read()
        self.assertNotIn("offer_recovery_if_any", source)

    def test_startup_parses_internal_smoke_test_flag(self):
        from run_v20 import _parse_args
        theme, project, smoke_test = _parse_args(["run_v20.py", "dark", "--smoke-test"])
        self.assertEqual(theme, "dark")
        self.assertIsNone(project)
        self.assertTrue(smoke_test)

    def test_startup_routes_agent_cli_commands(self):
        from run_v20 import _is_agent_cli_command
        self.assertTrue(_is_agent_cli_command(["run_v20.py", "compose", "--help"]))
        self.assertTrue(_is_agent_cli_command(["run_v20.py", "export", "--help"]))
        self.assertFalse(_is_agent_cli_command(["run_v20.py", "dark"]))
