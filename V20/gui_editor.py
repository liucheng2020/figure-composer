...
def _label_bounds_mm(layout, label_fontsize, label_offset, label_bold):
    """计算标签在导出画布中的真实边界，单位为毫米。"""
    font = fitz.Font(fontname="hebo" if label_bold else "helv")
    point_to_mm = 25.4 / 72.0
    baseline_x = layout.x - label_offset
    baseline_y = layout.y - label_offset
    text = str(layout.label)
    return (
        baseline_x,
        baseline_y - font.ascender * label_fontsize * point_to_mm,
        baseline_x + font.text_length(text, fontsize=label_fontsize) * point_to_mm,
        baseline_y - font.descender * label_fontsize * point_to_mm,
    )


def calculate_export_canvas(layouts, canvas_width, canvas_height, padding=2,
                            auto_crop=True, label_visible=True,
                            label_fontsize=12, label_offset=0.25,
                            label_bold=True):
    """Return shifted layout copies and an export canvas that contains all content.

    When auto_crop is True, blank canvas area is cropped to the figure and label bounds.
    When auto_crop is False, the original canvas size is preserved unless
    figures or labels overflow beyond it.
    """
    if not layouts:
    ...
    if label_visible:
        label_bounds = [
            _label_bounds_mm(layout, label_fontsize, label_offset, label_bold)
            for layout in layouts if str(layout.label)
        ]
        if label_bounds:
            min_left = min(min_left, min(bounds[0] for bounds in label_bounds))
            min_top = min(min_top, min(bounds[1] for bounds in label_bounds))
            max_right = max(max_right, max(bounds[2] for bounds in label_bounds))
            max_bottom = max(max_bottom, max(bounds[3] for bounds in label_bounds))
...
class ExportThread(QThread):
    ...
    def __init__(self, layouts, canvas_width, canvas_height, output_path,
                 export_format='pdf', dpi=300, label_bold=True,
                 auto_crop=False, label_offset=0.25, label_fontsize=12,
                 label_color=(0, 0, 0), label_visible=True):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            if self.export_format == 'pdf':
                ...
                export_combined_pdf(
                    export_layouts, crop_width, crop_height, self.output_path,
                    self.dpi, self.label_bold, self.label_offset,
                    self.label_fontsize, self.label_color, self.label_visible)
            elif self.export_format in ['png', 'tif']:
                from pdf_output import export_combined_image
                export_combined_image(
                    export_layouts, crop_width, crop_height, self.output_path,
                    self.export_format, self.dpi, self.label_bold,
                    self.label_offset, self.label_fontsize, self.label_color,
                    self.label_visible)
...
class FigureCombinerGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("学术组图工具 V20.0 FigBox")
        ...
        # Status bar
        self.statusBar().showMessage("就绪 | V20: GUI + Agent CLI | 拖拽PDF到画布 | Ctrl+C复制 Ctrl+V粘贴 | Ctrl+S保存项目")
    ...
    def create_menu_bar(self):
        ...
        act_settings.setStatusTip("设置 V20 默认画布、边距、导出、标签和自动备份")
    ...
    def create_toolbar(self):
        ...
        act_settings = QAction("设置", self)
        act_settings.setToolTip("打开 V20 默认设置")
        act_settings.triggered.connect(self.open_settings_dialog)
    ...
    def open_settings_dialog(self):
        ...
        self.statusBar().showMessage("V20 设置已保存", 5000)
    ...
    def export_with_options(self):
        ...
        label_color_rgb = (label_color.redF(), label_color.greenF(), label_color.blueF())
        self.export_thread = ExportThread(
            ...
            self.get_label_fontsize(),
            label_color_rgb,
            self.is_label_visible()
        )
    ...
    def export_all_formats(self):
        ...
        try:
            ...
            label_color_rgb = (label_color.redF(), label_color.greenF(), label_color.blueF())
            export_combined_pdf(
                export_states, crop_width, crop_height, pdf_path,
                self.get_dpi(), self.is_label_bold(), self.get_label_offset(),
                self.get_label_fontsize(), label_color_rgb, self.is_label_visible())
            ...
            export_combined_image(
                export_states, crop_width, crop_height, tif_path, 'tif',
                self.get_dpi(), self.is_label_bold(), self.get_label_offset(),
                self.get_label_fontsize(), label_color_rgb, self.is_label_visible())
            ...
            export_combined_image(
                export_states, crop_width, crop_height, png_path, 'png',
                self.get_dpi(), self.is_label_bold(), self.get_label_offset(),
                self.get_label_fontsize(), label_color_rgb, self.is_label_visible())
