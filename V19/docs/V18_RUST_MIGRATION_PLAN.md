...
文档日期: 2026-04-29
作者: V17 维护者
适用版本: 由 V17 (FigBox Container Edition) 出发，规划 V19 Rust 重构
...
3. 修改 V18 的 `pdf_output.py` / `pdf_boundary_fix.py` / `layout_engine.py`：
...
需要在 V17 上先跑出基线（建议在迁移启动前用 `cProfile` + `pytest-benchmark` 各跑一次保存到 `benches/baseline_v17.json`），然后 V19 每个 Phase 跑同样的数据集对照。

| 用例 | 数据集 | V17 基线（待测） | V19 目标 |
|---|---|---|---|
...
| **逐步迁移期间双格式维护** | 维护负担 | V17 / V19 共用 .figbox 容器（已设计），不需要双格式 |
...
- **.figbox 容器格式不变**：V19 直接读 V17 写的 .figbox，反之亦然
- **.figproj 仍能读**：V19 内置同样的 `import_legacy_figproj` 等价实现
- **所有命令行参数兼容**：`run_v17.py path/to/x.figbox` ↔ `figbox-v19 path/to/x.figbox`
