...
学术组图工具 V18 是一个面向科研论文组图的桌面软件，用于把 PDF、PNG、TIFF 等图像素材拖入画布后进行排版、对齐、标注和高分辨率导出。

V18 的核心目标是减少论文组图中反复缩放、对齐、铺满画布的手工操作，同时尽量保留每张图自身的宽高比例。

## Features

- 支持导入 PDF 和常见图片格式，在画布中自由拖拽、缩放和排列。
- 支持 `.figbox` / `.figproj` 项目文件，并兼容 V17 项目。
- 提供智能网格排版，可按指定行列整理选中图像。
...
```powershell
python run_v18.py dark
```

使用 cute 主题：

```powershell
python run_v18.py cute
```

直接打开已有项目：

```powershell
python run_v18.py "path\to\project.figbox"
```

指定主题并打开项目：

```powershell
python run_v18.py dark "path\to\project.figbox"
```

也可以在 Windows 中双击 `启动V18.bat` 启动。
...
├── auto_backup.py             # 自动备份
├── themes.py                  # 主题系统
├── build_exe_v18.py           # Windows EXE 打包脚本
├── tests/                     # 测试
├── docs/                      # 开发文档
└── V18更新说明.md              # V18 功能说明
```
...
不建议再通过复制 `V19`、`V20` 文件夹保存版本。新功能可以通过分支开发：
