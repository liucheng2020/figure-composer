...
V18 New Features (Proportional Fill Edition):
...
6. 完全兼容 V17 的 .figbox / .figproj 项目文件
...
class CanvasView(QGraphicsView):
    ...
    def start_fill_line(self, callback):
        """V18: 进入画线模式。用户拖出一条横线后，回调 callback(x_left, x_right, y)（场景坐标=mm）。"""
    ...
    def mouseMoveEvent(self, event):
        ...
        # V18: 画线模式 —— 拖动时实时更新横线（约束为水平线，y 固定为起点 y）
    ...
    def mouseReleaseEvent(self, event):
        ...
        # V18: 画线模式 —— 松开左键即完成，回调宽度信息后退出画线模式
...
class FigureCombinerGUI(QMainWindow):
    ...
    def create_toolbar(self):
        ...
        # V18: 智能网格 - 选中图按 行×列 排列，每行用统一系数等比缩放铺满画布宽度
    ...
    def create_parameters_dock(self):
        ...
        # V18: 导出默认 1000 DPI（出版级）；屏幕预览仍用 72 DPI 渲染，保证流畅
    ...
    def import_dropped_pdfs(self, pdf_paths, drop_pos=None):
        ...
        Args:
            ...
            drop_pos: V18 - 拖放落点 (x_mm, y_mm)，图的中心落在此处；None 则居中
        """
        try:
            imported_count = 0
            ...
            for pdf_path in pdf_paths:
                ...
                # V18: 落在拖放位置（图的中心对准落点）；多张时依次错开避免完全重叠。
                # 未提供落点（如剪贴板粘贴）则居中。
    ...
    def import_dropped_files(self, file_paths, drop_pos=None):
        ...
        Args:
            ...
            drop_pos: V18 - 拖放落点 (x_mm, y_mm)，图将落在此处；None 则居中
        """
        try:
            pdf_files = []
    ...
    def renumber_labels(self):
        """V18: 按"现有标签顺序"重新编号为连续 A、B、C…（删除后保持顺序与连续性）。"""
    ...
    def relabel_insert(self, moved_item, new_label):
        """V18 标签级联：把 moved_item 移动到 new_label 所在的位置，其余标签自动顺延。
    ...
    def load_pdfs(self):
        ...
        try:
            ...
            if self.pdf_files:
                # V18: 导入后给出"全局统一缩放换行"的初始铺排（按文件顺序 = 标签 A、B、C…）
                engine = LayoutEngine(
                    self.get_canvas_width(),
                    self.get_canvas_height(),
                    self.get_margin(),
    ...
    # ==================================================================
    # V18 排版：智能网格（每行统一系数等比缩放，从左铺满画布宽度）
    # ==================================================================
    ...
    def apply_smart_grid_layout(self):
        """V18 智能网格：选中图按 行×列 排列；可选「等高填充」到鼠标画出的宽度。
    ...
    def arrange_items_in_grid(self, items, rows, cols):
        """V17 原版智能网格：把图按 行×列 摆成网格，单元格按最大图尺寸对齐，不改变图片大小。"""
        if not items:
            return
